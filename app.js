...
function buildClinicSection(data, total, title = "การจัดบริการคลินิกพิเศษเฉพาะทางนอกเวลาราชการที่ท่านต้องการให้เปิดบริการ") {
...
function buildPayExtraSection(data, total) {
  const payOptions = [
  ...
  let html = renderSectionHeader("ส่วนที่สมัครใจจ่ายเพิ่ม");
...
function buildPreferredLocationSection(data, total) {
  ...
  let html = renderSectionHeader("สถานที่ต้องการให้จัดบริการ");
...
function renderPersonnelDemographicsTable(data) {
  ...
  let html = `
    ...
    ${renderSectionHeader("3. ประเภทบุคลากร")}
    ${renderCountRow("ข้าราชการ", types["ข้าราชการ"] || 0, n)}
    ${renderCountRow("พนักงานราชการ", types["พนักงานราชการ"] || 0, n)}
    ${renderCountRow("พนักงานกระทรวงสาธารณสุข", types["พนักงานกระทรวงสาธารณสุข"] || 0, n)}
    ${renderCountRow("ลูกจ้างชั่วคราว", types["ลูกจ้างชั่วคราว"] || 0, n)}
    ${renderCountRow("ลูกจ้างประจำ", types["ลูกจ้างประจำ"] || 0, n)}
  `;

  const otherTypeTexts = data.map(item => item.TypeOther).filter(Boolean);
  const otherTypeCount = otherTypeTexts.length;
  html += renderCountRow(`อื่นๆ (${summarizeTextValues(otherTypeTexts)})`, otherTypeCount, n);
  html += renderSectionHeader("4. ตำแหน่งสายงาน");
  const sortedPositions = Object.keys(positions)
    .filter(key => !key.startsWith("_"))
    .sort((a, b) => (positions[b] || 0) - (positions[a] || 0));
  if (sortedPositions.length === 0) {
    html += `<tr><td colspan="3" style="text-align:center;color:var(--text-light);">ยังไม่มีข้อมูลตำแหน่งสายงาน</td></tr>`;
  } else {
    sortedPositions.forEach(pos => {
      html += renderCountRow(pos, positions[pos], n);
    });
  }
  if (positions._otherCount > 0) {
    html += renderCountRow(`อื่นๆ (${summarizeTextValues(Object.keys(positions._otherValues || {}))})`, positions._otherCount, n);
  }
  html += renderSectionHeader("5. รายได้รวมต่อเดือนของครอบครัว");
  html += renderCountRow("ต่ำกว่า 10,000 บาท", incomes["ต่ำกว่า 10,000 บาท"] || incomes["ต่ำกว่า 10,000"] || 0, n);
  html += renderCountRow("10,001 – 30,000 บาท", incomes["10,001 – 30,000"] || 0, n);
  html += renderCountRow("30,001 – 50,000 บาท", incomes["30,001 – 50,000"] || 0, n);
  html += renderCountRow("50,001 – 70,000 บาท", incomes["50,001 – 70,000"] || 0, n);
  html += renderCountRow("70,001 – 90,000 บาท", incomes["70,001 – 90,000"] || 0, n);
  html += renderCountRow("มากกว่า 90,000 บาท", incomes["มากกว่า 90,000 บาท"] || incomes["มากกว่า 90,000"] || 0, n);
...
function renderPersonnelOpinionsTable(data) {
  ...
  let html = `
    ...
    ${renderSectionHeader("1. ท่านเห็นด้วยหรือไม่กับการจัดบริการคลินิกพิเศษเฉพาะทางนอกเวลาราชการ")}
    ${renderCountRow("เห็นด้วย", agreeSMC["เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่เห็นด้วย", agreeSMC["ไม่เห็นด้วย"] || 0, n)}
    ${renderSectionHeader("2. คาดว่าจะได้รับบริการรวดเร็วขึ้นหรือไม่")}
    ...
    ${renderSectionHeader("3. ท่านจะใช้บริการหรือไม่")}
    ...
    ${renderSectionHeader("5. จะทำให้เลือกแพทย์เฉพาะทางได้สะดวกกว่า")}
    ...
    ${renderSectionHeader("6. จะทำให้ลดระยะเวลารอคอยคิวผ่าตัดได้")}
    ...
    ${renderSectionHeader("7. ช่วงเวลาที่สะดวกรับบริการ")}
    ...
    ${renderSectionHeader("8. ความยินดีที่จะจ่ายค่าบริการเพิ่ม")}
...
function renderPublicOpinionsTable(data) {
  ...
  let html = `
    ...
    ${renderSectionHeader("4. หากมีการจัดบริการคลินิกพิเศษเฉพาะทางนอกเวลาราชการ จะทำให้เลือกแพทย์เฉพาะทางได้สะดวกกว่า")}
    ...
    ${renderSectionHeader("5. คาดว่าจะได้รับบริการรวดเร็วขึ้นหรือไม่")}
    ${renderCountRow("เห็นด้วย/รวดเร็วขึ้น", fastService["เห็นด้วย/รวดเร็วขึ้น"] || 0, n)}
    ${renderCountRow("ไม่เห็นด้วย", fastService["ไม่เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่แน่ใจ", fastService["ไม่แน่ใจ"] || 0, n)}
    ${renderSectionHeader("6. ท่านจะใช้บริการหรือไม่")}
    ${renderCountRow("ใช้บริการ", willUse["ใช้บริการ"] || 0, n)}
    ${renderCountRow("ไม่ใช้บริการ", willUse["ไม่ใช้บริการ"] || 0, n)}
    ${renderCountRow("ไม่แน่ใจ", willUse["ไม่แน่ใจ"] || 0, n)}
    ${buildClinicSection(data, n)}
    ${renderSectionHeader("8. จะทำให้เลือกแพทย์เฉพาะทางได้สะดวกกว่า")}
    ${renderCountRow("เห็นด้วย", convenientDoctor["เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่เห็นด้วย", convenientDoctor["ไม่เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่แน่ใจ", convenientDoctor["ไม่แน่ใจ"] || 0, n)}
    ${renderSectionHeader("9. จะทำให้ลดระยะเวลารอคอยคิวผ่าตัดได้")}
    ${renderCountRow("เห็นด้วย", reduceQueue["เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่เห็นด้วย", reduceQueue["ไม่เห็นด้วย"] || 0, n)}
    ${renderCountRow("ไม่แน่ใจ", reduceQueue["ไม่แน่ใจ"] || 0, n)}
    ${renderSectionHeader("10. ช่วงเวลาที่สะดวกรับบริการ")}
    ${renderCountRow("กลางวัน", convenientTime["กลางวัน"] || 0, n)}
    ${renderCountRow("ตอนเย็น", convenientTime["ตอนเย็น"] || 0, n)}
    ${renderCountRow("กลางคืน", convenientTime["กลางคืน"] || 0, n)}
    ${renderCountRow("ตลอดทั้งวัน", convenientTime["ตลอดทั้งวัน"] || 0, n)}
    ${renderSectionHeader("11. ความยินดีที่จะจ่ายค่าบริการเพิ่ม")}
    ${renderCountRow("ยินดีจ่ายเพิ่ม", payExtra["ยินดีจ่ายเพิ่ม"] || 0, n)}
    ${renderCountRow("ไม่ยินดีจ่ายเพิ่ม", payExtra["ไม่ยินดีจ่ายเพิ่ม"] || 0, n)}
    ${renderCountRow("ไม่แน่ใจ", payExtra["ไม่แน่ใจ"] || 0, n)}
    ${buildPayExtraSection(data, n)}
